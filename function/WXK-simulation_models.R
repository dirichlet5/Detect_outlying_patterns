...
plot_dtt <- function(y, grid_points, p, true_outliers, show_legend, plot_title,
  ...
  if(length(true_outliers) > 0){
    ...
    plot(x = grid_points, type = "n", ylab = ylabel, xlab = xlabel,
         ...
         col.lab = "gray20",  cex.lab=2, axes = F)
  ...
  axis(1, col = "white", col.ticks = "grey61",
       at=c(0,10,20,30,40,50), labels=c(0,0.2,0.4,0.6,0.8,1),
       lwd.ticks = .5, tck = -0.025,
       cex.axis = 2, col.axis = "gray30")
  ...
  if(show_legend){
    legend(legend_pos, legend = c("normal", "outlier"),
           ...
           box.lwd = .1, xjust = 0,cex=1.5, inset = .01)
  }
...
simulation_model10 <- function(n = 100, p = 50, outlier_rate = 0.05,
                              mu = 10, q = 8, kprob = 0, a = 0.1, b = .9, l = 0.05,
                              cov_alpha = 1, cov_beta = 1, cov_nu = 1,
                              deterministic = TRUE, seed = NULL,
                              plot = F,
                              plot_title = "Simulation Model 10",
                              title_cex = 1.5,
                              show_legend = T,
                              ylabel = "",
                              xlabel = "gridpoints"){
...
simulation_model02 <- function(dt, n = 100, p = 50, outlier_rate = 0.05,
                              mu = 4, q = 8, kprob = 0.5, a = 0.1, b = .9, l = 0.05,
                              cov_alpha = 1, cov_beta = 1, cov_nu = 1,
                              deterministic = TRUE, seed = NULL,
                              plot = F,
  ...
  if(n_outliers > 0){
    ...
    qcoeffk <- qcoeffk*q
    indicator <- sapply(runif(n_outliers, a, b), function(x) (tt >= x)*(tt <= x + l) )
    y[, true_outliers] <- dt + (indicator*rep(qcoeffk, rep(p, n_outliers)))
...
simulation_model06 <- function(dt, n = 100, p = 50, outlier_rate = .1,
                              mu = 4, q = 1.8, kprob = 0.5,
                              a = .25, b = .75,
                              cov_alpha = 1, cov_beta = 1, cov_nu = 1,
                              pi_coeff = 0.02, exp_pow = 2,
