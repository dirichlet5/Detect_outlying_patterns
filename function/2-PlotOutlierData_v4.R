...
aa <- list(dt1$data,dt2$data,dt3$data,dt4$data)
bb <- list(dt1,dt2,dt3,dt4)
MISS_PER <- 0.65
p<-length(aa)
sparse_data = multi_sparse(aa, p_size = rep(1, p),
                           p_curve = rep(MISS_PER, p),
                           sparsity = c("point", "point", "point","point"))
observed_curve = aa
TYPE = 3 #Specify the selected Outlier type here!
