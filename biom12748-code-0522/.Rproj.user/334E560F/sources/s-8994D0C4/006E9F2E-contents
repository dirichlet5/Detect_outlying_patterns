...
observeX<-rawX+measurementerror  ##  W_{im}

X<-matrix(NA,n,p)  ##  It leads to a smoothed estimate of Xi(s)[W_{im}],Step I
for (i in 1:n){
...
parameter<-exp(rawX%*%beta*calh+Z%*%gamma)  ## h(t),hazard function
...
time<-failuretime*event+censoringtime*(rep(1,n)-event)  ## min(failure_time, censoring_time)
...
totaleigenscore<-scaleX%*%(Eigvec/sqrt(h))*h  ## Step II
...
eigenscore<-scaleX%*%est_Eigfun*h  ## estimated FPCs scores...

designmatrix<-cbind(eigenscore,Z)  ## Q(eta)?
...
fullcoxresult<-coxph(formula=totalformular, data=newdataframe)  # fit a typical Cox regression model

cox.zph(fullcoxresult)
ggcoxzph(cox.zph(fullcoxresult))
